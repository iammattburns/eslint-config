...
- You can see all the [rules here](https://github.com/iammattburns/eslint-config/blob/main/.eslintrc.js)
...
```
npm install @iammattburns/eslint-config
```
...
```json
"eslintConfig": {
  "extends": ["@iammattburns/eslint-config"]
}
```
...
For TypeScript projects, use `@iammattburns/eslint-config/typescript`.

```json
{
  "extends": ["@iammattburns/eslint-config/typescript"]
}
```
...
1. Run `npx install-peerdeps --dev @iammattburns/eslint-config`
1. Crack open your `package.json` and replace `"extends": "react-app"` with `"extends": "@iammattburns/eslint-config"`
...
Same instructions as above, just make sure you extend `@iammattburns/eslint-config/typescript` instead of just `@iammattburns/eslint-config`.
